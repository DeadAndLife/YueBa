...
-(NSURLSessionDataTask *)loginAccount:(NSString *)phoneNumber Pwd:(NSString *)pwd CompletionHandle:(QYHTTPSessionManagerCompletionHandle)completionHandle{
    ...
    return [self POST:kLoginAPI parameters:params CompletionHandle:^(id responseObject, NSURLSessionTask *task, NSError *error) {
        completionHandle(responseObject, task, error);
    }];
}
...
-(NSURLSessionDataTask *)uploadUserInfo:(NSDictionary *)userInfo CompletionHandle:(QYHTTPSessionManagerCompletionHandle)completionHandle{
    ...
    return [self POST:kUploaduserInfoAPI parameters:userInfo CompletionHandle:^(id responseObject, NSURLSessionTask *task, NSError *error) {
        completionHandle(responseObject, task, error);
    }];
}
